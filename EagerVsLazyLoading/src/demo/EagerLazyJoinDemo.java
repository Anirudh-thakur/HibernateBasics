...
public class EagerLazyJoinDemo {
	...
	public static void main(String[] args) {
		...
		try
		{
			...
			Query<InstructorMany> query = session.createQuery("select i from InstructorMany i "
				         +"JOIN FETCH i.courses "
					+"where i.id=:theInstructorId",InstructorMany.class);
			query.setParameter("theInstructorId", id);
			
			//execute query 
			InstructorMany tempInstructor = query.getSingleResult();
